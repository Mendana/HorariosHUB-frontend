...
export function useProposals(
  ...
  //       ? `/api/proposals/mine?page=${page}&limit=${PAGE_SIZE}`
  //       : `/api/proposals?status=${status}&page=${page}&limit=${PAGE_SIZE}`;
  //     return apiFetch<{ data: Proposal[]; total: number }>(url);
  //   },
  // });
  ...
  //     apiFetch(`/api/proposals/${id}/reject`, { method: 'PATCH', body: reason ? JSON.stringify({ reason }) : undefined }),
  //   onSuccess: () => qc.invalidateQueries({ queryKey: ['proposals'] }),
  //   onError: (err) => toast.error(getErrorMessage(err)),
  // });
  // return {
  ...
  throw new Error('MOCKS.proposals is false pero la implementación real no está conectada. Ver docs/CONNECTING_BACKEND.md');
}
